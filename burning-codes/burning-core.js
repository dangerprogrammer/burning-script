                ...
                if (player && gameData) {
                    ...
                    if (gameState)
                        if (activeUnit) {
                            ...
                            var h = UTILS.getDirection(k, f, bot.botPlayer.x - d, bot.bo.y - c);
                            0 == activeUnit.type ? (b = UTILS.getDistance(k, f,
                                    bot.bo.x - d, bot.bo.y - c), b - activeUnit.size < bot.bo.startSize ? (k = bot.bo.x - d + (activeUnit.size + bot.bo.startSize) * MathCOS(h), f = bot.bo.y - c + (activeUnit.size + bot.bo.startSize) * MathSIN(h)) : b + activeUnit.size > bot.bo.buildRange - .15 && (k = bot.bo.x - d + (bot.bo.buildRange - activeUnit.size - .15) * MathCOS(h), f = bot.bo.y - c + (bot.bo.buildRange - activeUnit.size - .15) * MathSIN(h))) : 1 == activeUnit.type || 2 == activeUnit.type ? (k = bot.bo.x - d + (activeUnit.size + bot.bo.buildRange) * MathCOS(h), f = bot.bo.y - c + (activeUnit.size + bot.bo.buildRange) * MathSIN(h)) :
                                3 == activeUnit.type && (b = UTILS.getDistance(k, f, bot.bo.x - d, bot.bo.y - c), b - activeUnit.size < bot.bo.startSize ? (k = bot.bo.x - d + (activeUnit.size + bot.bo.startSize) * MathCOS(h), f = bot.bo.y - c + (activeUnit.size + bot.bo.startSize) * MathSIN(h)) : b + activeUnit.size > bot.bo.buildRange + 2 * activeUnit.size && (k = bot.bo.x - d + (bot.bo.buildRange + activeUnit.size) * MathCOS(h), f = bot.bo.y - c + (bot.bo.buildRange + activeUnit.size) * MathSIN(h)));
                            activeUnitDir = h;
                            ...
                            if (0 == activeUnit.type || 2 == activeUnit.type || 3 == activeUnit.type)
                                for (e = 0; e < units.length; ++e)
                                    if (1 != units[e].type && units[e].owner == bot.bo.sid && 0 <= activeUnit.size + units[e].size - UTILS.getDistance(k, f, units[e].x - d, units[e].y - c)) {
                    ...
                    else activeBase && (mainContext.fillStyle = outerColor, renderCircle(bot.bo.x - d, bot.bo.y - c, bot.bo.size + 50, mainContext, !0));
                    if (selUnits.length)
                    ...
                    activeUnit && renderUnit(k, f, h, activeUnit, playerColors[bot.bo.color], mainContext);
                    ...
                    hoverUnit ? toggleUnitInfo(hoverUnit) : activeBase ? toggleUnitInfo(activeBase, activeBase.sid == bot.bo.sid) : activeUnit ? toggleUnitInfo(activeUnit) :
